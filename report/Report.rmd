...
packages <- c("ggplot2", "readr", "ggpubr", "pander", "ggbiplot", "FSA", "latex2exp", "xtable")
...
# Results
## Demographics
The patients were divided by diagnosis and the PDAC patients are further seperated by the stage of the disease. The number of samples per diagnosis and stage are shown below.
```{r}
...
\newpage
## REG1B outperforms REG1A
Though the performance of REG1A and REG1B are very similar, REG1B outperformed REG1A when control samples were compared to stage I-IIA PDAC samples (Kruskal-Wallis test; p < 0.0003). [\ref{fig:fig1}]

```{r reg-vs, fig.cap = "\\label{fig:fig1}P-values of Kruskal-Wallis test, Dunn's multiple comparisons over 306 urine samples"}
ggplot(REG1A$res[c(1,2,4),], aes(x = Comparison, y = P.adj)) +
  ...
  labs(y = "P-value (adjusted)", title = "Comparison REG1A and REG1B",
       caption = "(166 Control/Benign, and 140 PDAC [16 stage I-IIA, 54 stage I-II, 70 stage III-IV])") +
